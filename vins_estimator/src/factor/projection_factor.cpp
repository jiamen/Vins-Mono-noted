...
// CostFunction::Evaluate()：计算误差、Jacobian
bool ProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    // 地图点在i帧相机坐标系下坐标
    Eigen::Vector3d pts_camera_i = pts_i / inv_dep_i;    // inv_dep_i = λ = 1/z
    // 转成第i帧imu坐标系
    ...
    if (jacobians)
    {
        ...
        Eigen::Matrix3d Rj = Qj.toRotationMatrix();     // R_w_bj
...
#else
        // 重投影误差对j帧相机坐标系下坐标求导，这个公式就是《VIO第3讲》67页的公式  链式法则 ∂r_c / ∂f_cj，后面使用f_cj对状态量进行求导
        reduce << 1. / dep_j, 0, -pts_camera_j(0) / (dep_j * dep_j),
            0, 1. / dep_j, -pts_camera_j(1) / (dep_j * dep_j);
#endif
        reduce = sqrt_info * reduce;

        if (jacobians[0])       // 《VIO第3讲》P63页   fcj对i时刻的状态量求导
        {
            Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>> jacobian_pose_i(jacobians[0]);     // Twb

            Eigen::Matrix<double, 3, 6> jaco_i;
            jaco_i.leftCols<3>() = ric.transpose() * Rj.transpose();        // P63页 公式（64） 前三维是对平移求导
            jaco_i.rightCols<3>() = ric.transpose() * Rj.transpose() * Ri * -Utility::skewSymmetric(pts_imu_i); // P64页 公式（67） 后三维是对旋转求导

            jacobian_pose_i.leftCols<6>() = reduce * jaco_i;
            jacobian_pose_i.rightCols<1>().setZero();
        }

        if (jacobians[1])       // 《VIO第3讲》P65页   fcj对j时刻的状态量求导
        {
            Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>> jacobian_pose_j(jacobians[1]);     // Twb
        ...
        if (jacobians[2])
        {
            Eigen::Map<Eigen::Matrix<double, 2, 7, Eigen::RowMajor>> jacobian_ex_pose(jacobians[2]);    // Tbc
        ...
        if (jacobians[3])
        {
            Eigen::Map<Eigen::Vector2d> jacobian_feature(jacobians[3]);     // λ
#if 1
